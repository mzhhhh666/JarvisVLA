...
class VLLM_AGENT:
    def __init__(self,checkpoint_path, base_url, api_key="EMPTY",
        ...
        try:
            ...
            print(f"[INFO] Successfully connected to vLLM API at {base_url}. Model ID: {self.model}")
        except Exception as e:
            print(f"[ERROR] Failed to connect to vLLM API at {base_url}: {e}")
            # 您可以选择在这里重新抛出异常，或者进行其他错误处理
            raise
    ...
    def forward(self,observations,instructions,verbos=False,need_crafting_table=False):
        ...
        try:
            ...
            print(f"[INFO] Successfully received response from model API.")
        except Exception as e:
            print(f"[ERROR] Error during model inference API call: {e}")
            # 根据需要处理错误，例如返回一个默认动作或重新抛出异常
            raise
